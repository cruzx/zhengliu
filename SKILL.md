---
name: zhengliu
description: '人类蒸馏企划：专注蒸馏人物视角、设计判断方式、设计评审逻辑与方案收敛方法，生成可运行的设计师导向Skill。

  适用入口：(1)明确人名/角色/协作对象 (2)明确评审主题，如“提炼过稿逻辑”“沉淀设计判断方式”。

  推荐触发词：「蒸馏XX」「做个XX skill」「做个XX视角」「更新XX的skill」「沉淀设计判断方式」「提炼过稿风格」「提炼设计评审逻辑」。

  不建议仅用泛词单独触发，如：「提炼」「总结」「分析一下」。'
description_zh: 蒸馏人物视角、设计判断与设计评审逻辑，生成可运行的设计师导向 Skill
description_en: Distill people, design judgment, and design review logic into a runnable design-oriented skill
version: 1.1.0
...
当任务属于设计评审、过稿收敛、设计判断方式提炼、概念版/商业化/增长设计、产品体验反馈时，默认切到 **设计师优先模式**。

这个模式和普通人物蒸馏的区别是：
- 不只提炼“这个人怎么想”，还提炼“这个人怎么收稿、怎么判断优劣、怎么给过稿结论”
- 不只看审美，还拆成 `视觉风格 / 问题定义 / 证据标准 / 向上汇报方式 / 业务判断`
- 不默认用户要长分析，而是先判断最终要回的是：`图 / 标注 / 改稿建议 / 策略判断 / 面向未来的判断`

### 设计师优先模式的默认规则

1. 先判断用户到底要什么交付，不要一上来长篇解释。
2. 如果用户已经给了图、稿、页面或交互稿，先看当前稿，再决定是否提问。
3. 提炼设计人物时，默认优先抽取：
- 他如何定义问题
- 他如何判断一版稿是否能过
- 他如何评价工作成果优劣
- 他如何平衡趣味、质感、业务目标和落地成本
- 他如何向上沟通
4. 提炼评审语境或协作判断方式时，默认额外回答：
- 当前评审语境对业务的判断标准是什么
- 当前协作语境对工作成果优劣的判断标准是什么
- 当前协作关系里，长期被反复强调的要求是什么
5. 设计任务如果属于概念版、商业化、增长设计、产品体验反馈等明确语境，优先吸收对应历史语境，而不是把人物蒸馏成脱离业务的抽象审美人格。
6. 当用户要的是直接执行，优先输出可行动结果，不要只停在人物分析。
7. 如果任务涉及会员、权益、购买、下载、拦截、弹窗、收银台、券包、资格判断，默认先拆 `用户状态 → 对应跳转 → 对应售卖内容`，不要把所有用户当成同一条链路。

### 设计师优先模式的会员状态分流检查

当需求涉及会员权益、下载权益、购买弹窗、资格拦截、MP3下载券、特权入口等场景时，默认把“不同会员情况的跳转”当成 P0 检查项。
...
- 默认链路：跳转 `特权拦截弹窗`
- 目标动作：引导 `开会员`
- 检查重点：是不是在卖会员权益，而不是误卖单次券包

2. `会员，但没券`
- 默认链路：拉起 `MP3下载购买弹窗`
- 目标动作：补足下载资格
- 检查重点：是不是直接卖缺失能力，而不是重复卖会员

3. `有券`
- 默认链路：`直接拉下载`
- 目标动作：减少打断，直接完成任务
- 检查重点：是否被多余弹窗、售卖或教育链路打断

设计评审时，至少追问并判断：
- 这个入口是否根据不同会员状态做了差异化响应
- 同一个点击动作背后，是否其实对应多条不同的商业化/体验链路
- 文案、按钮和弹窗是不是在卖对的东西
- 是否会让会员用户误以为还要重复购买会员
- 是否会让已有资格的用户被多余打断
- 当前问题到底是 `入口设计问题`，还是 `资格分流没做清楚`

### 设计师优先模式的输出分流

进入设计师优先模式后，先判断最终交付属于哪类：

| 交付类型 | 用户常见表达 | 默认产出 |
|---------|-------------|---------|
| 图 / 改稿 | 「帮我出稿」「画一下」「标出来」「直接改」 | 图上标注、改稿方向、交互收法 |
| 评审建议 | 「帮我看看」「为什么不过」「怎么收」 | P0/P1/P2 问题、过稿建议、优先级 |
| 策略判断 | 「这个值不值得做」「这套方向会不会被认可」 | 判断依据、业务标准、风险 |
| 未来判断 | 「以后该往哪走」「长期要沉淀什么」 | 方法论、能力沉淀、未来方向 |
...
| 用户输入 | 路径 | 示例 |
|---------|------|------|
| 明确的人名/主题 | **直接路径** → Phase 0A | 「蒸馏克鲁斯」「做一个克鲁斯skill」 |
...
6. **最终交付是什么**：这次你要我最后回的是一张图、图上标注、过稿建议、方案方向，还是一个面向未来的判断？

用户说「就做XX」没有更多信息 → 默认全面画像 + 思维顾问 + 无本地语料（走网络搜索），直接推进。
用户提供了本地语料 → 标记为**本地语料模式**，Phase 1的采集策略会相应调整。

如果任务明显属于设计师语境，默认补充这些隐式判断：
- 这次蒸馏对象更像 `审美型人物`、`过稿型人物`、`业务判断型协作对象`，还是 `综合设计判断者`
- 最终交付更偏 `直接出图` 还是 `先给判断`
...
用户不知道该蒸馏谁，只有需求或困惑。这时人类蒸馏企划的工作是**从需求反推最合适的蒸馏对象**。
...
| 设计评审与过稿 | 「这版为什么不过」「当前评审到底看什么」「怎么收更容易被拍板」 | 过稿逻辑、判断方式、结构化收敛、业务/质感平衡 |
...
人类蒸馏企划：你说的决策主要是哪种场景？比如商业/投资决策，还是职业/人生方向的选择？

用户：主要是商业上的，比如要不要做某个产品、要不要接某个合作

人类蒸馏企划：明白了，你的核心需求是「在信息不完整时快速做出高质量的商业判断」。
我推荐3个候选：
[展示候选推荐...]
```
...
如果是设计场景，推荐时优先区分这几类候选：
- `视觉风格型`：适合做审美方向和气质判断
- `过稿收敛型`：适合做评审、改稿、向上沟通
- `业务判断型`：适合做商业化、产品、增长取舍
- `综合型协作角色`：适合提炼“评审看什么、怎么拍板”
...
| 聊天截图/评审记录 | 提取判断标准、评审话术、过稿逻辑 | 对话(02)、决策(05)、表达(03) |
| 设计稿/标注图/改稿前后 | 提取优劣判断、收稿方式、质感标准 | 决策(05)、表达(03)、他者(04) |
...
如果目标人物是设计师、评审者、协作对象或过稿型角色，6个 Agent 在标准任务外还要额外提取：
- 他对 `首屏清晰度` 的要求
- 他对 `关键入口突出度` 的要求
- 他对 `利益点露出` 的要求
- 他如何评价一版稿 `太弱 / 太飘 / 不耐看 / 不贴场景`
- 他更常要求的是 `出图`、`重画`、`收法`、`汇报结构`，还是 `未来方向判断`
- 他如何平衡 `趣味感 / 质感统一 / 业务目标 / 落地成本`
...
如果蒸馏对象属于设计师或关键协作对象，再额外提炼：

| 维度 | 提取内容 |
|------|---------|
| 问题定义方式 | 他先看首屏、入口、利益点、业务目标，还是视觉氛围 |
| 过稿判断标准 | 什么情况下会说 `可以过`、`还弱`、`不成立` |
| 证据标准 | 他更依赖截图、数据、竞品、用户反馈还是历史语境 |
| 向上沟通方式 | 他是短促拍板型，还是结构化说明型 |
| 交付偏好 | 他更偏好图、标注、方案、还是未来判断 |
...
| 创建者归属 | 固定内容：`> 本Skill由 [人类蒸馏企划 · Skill框架术]> 创建者：Cruz` |

如果目标对象属于设计师 / 评审者 / 关键协作对象，还应补充这些 section：
- `过稿工作流`
- `设计判断方式`
- `工作成果优劣判断方式`
- `更适合输出图、标注、方案还是未来判断`
...
如果是设计师语境，再额外检查：
- 有没有把人物拆成 `视觉风格 / 问题定义 / 证据标准 / 向上沟通风格`
- 有没有回答当前评审语境对业务和成果的判断方式
- 有没有回答用户到底要 `图 / 标注 / 方案 / 判断 / 未来判断`
- 有没有把“怎么改”落到具体，而不是只给抽象评价
...
如果是设计师 / 关键协作对象蒸馏，额外要求：
- 能说清 `什么样的稿更容易过`
- 能说清 `什么样的成果会被判定为弱`
- 能判断 `这次该回图还是该回未来判断`
- 能判断 `是否需要先拆非会员 / 会员无券 / 有券` 再评界面和链路
...
1. 人类蒸馏企划无法从公开渠道搜到用户的思维框架，需要用户提供素材
...
人类蒸馏企划不是减少人类，而是让人类在历史上更有作为。

一个好的人物Skill，让你用另一个人的眼睛看自己的问题。不是快速过稿，而是为了拓展你自己的思维边界。
