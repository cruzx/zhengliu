...
当任务属于设计评审、过稿收敛、leader 判断方式提炼、概念版/商业化/增长设计、产品体验反馈时，默认切到 **设计师优先模式**。
...
- 他如何向上沟通
4. 提炼 leader 时，默认额外回答：
- leader 对业务的判断标准是什么
- leader 对工作成果优劣的判断标准是什么
- leader 对我工作的长期评价是什么
...
| 策略判断 | 「这个值不值得做」「leader会不会买单」 | 判断依据、业务标准、风险 |
...
- 这次蒸馏对象更像 `审美型人物`、`过稿型人物`、`业务判断型 leader`，还是 `综合设计判断者`
- 最终交付更偏 `直接出图` 还是 `先给判断`
...
| 设计评审与过稿 | 「这版为什么不过」「leader到底看什么」「怎么收更容易被拍板」 | 过稿逻辑、leader标准、结构化收敛、业务/质感平衡 |
...
- `综合型 leader`：适合提炼“leader 看什么、怎么拍板”
...
| 聊天截图/评审记录 | 提取判断标准、leader话术、过稿逻辑 | 对话(02)、决策(05)、表达(03) |
...
如果目标人物是设计师、leader、评审者或过稿型角色，6个 Agent 在标准任务外还要额外提取：
- 他对 `首屏清晰度` 的要求
- 他对 `关键入口突出度` 的要求
- 他对 `利益点露出` 的要求
...
如果目标对象属于设计师 / 评审者 / leader，还应补充这些 section：
- `过稿工作流`
- `leader 判断标准`
- `工作成果优劣判断标准`
- `更适合输出图、标注、方案还是未来判断`
